...
class Particle:
    ...
    def update_position(self):
        ...
        for _ in range(len(self.position) - 1):
            ...
            if self.start_type == 'M' and self.current_location == self.start_location:
                ...
                next_location = self.select_next_location(remaining_locations)
    ...
    def calculate_probabilities(self, remaining_locations):
        probabilities = {}
        total_probability = 0
        ...
        for location in self.unserviced_locations | self.distribution_centers:
            ...
            probability = abs(self.velocity[self.position.index(location)])

            print(probability)
            if location in self.unserviced_locations:
                required_load = self.location_demands[location]
                if self.current_load >= required_load:
                    probability *= 3.0 # Increase the prob we supply to them
                else:
    ...
    def select_next_location(self, remaining_locations):
        probabilities = self.calculate_probabilities(remaining_locations)
